...
int
load_xresources_i(Display *display,ResourcePref *p)
{
	char *resm;
	XrmDatabase db;
	...
	if (!resm)
		return -1;
	int minw = ((XSizeHints*)display)->min_width;
...
//		die("dwm: cannot get xcb connection\n");
	Display *dpy;
	ResourcePref *p;
	char *version__ =malloc(sizeof(char)*strlen("x.x.x"));
	sprintf(version__, "dwm-%s", VERSION);
	if (argc == 2 && !strcmp("-v", argv[1]))
		die(version__);
	int ret = load_xresources_i(dpy,p);
